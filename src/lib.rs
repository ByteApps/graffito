...
fn note_web_url(network: Network, address: &str, note_id: &str) -> String {
    match network {
        ...
        net => format!(
            "https://objsal.github.io/graffito/companion/note.html?address={address}&network={}&note={note_id}",
            net.as_str()
        ),
    }
}
        ...
        if let Some(n) = store.notes.iter().find(|n| n.note_id.as_str() == id.as_str()) {
            ...
            let web = match s.network {
                ...
                net => {
                    ...
                    format!(
                        "https://objsal.github.io/graffito/companion/note.html?address={addr}&network={}&note={}",
                        net.as_str(),
                        n.note_id
                    )
                }
