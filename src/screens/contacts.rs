...
pub(crate) fn on_pick_contact(&mut self, w: &AppWindow, addr: SharedString) {
        ...
        if w.global::<Ui>().get_pick_mode().as_str() == "sweep" {
            ...
            if Recipient::parse(self.network, &a).is_err() {
                let lower = a.to_lowercase();
                if Recipient::parse(self.network, &lower).is_ok() {
                    a = lower;
                } else {
                    println!("cb: sweep-pick err=bad-address");
                    w.global::<Ui>().set_status(format!("not a valid {} address", self.network.as_str()).into());
                    return;
                }
            }
