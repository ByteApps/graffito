...
//! concatenated (digit-mode QR) — 48 digits for 12 words, 96 for 24.
//! Compact: the raw entropy bytes (byte-mode QR) — 16 or 32 bytes.
...
pub fn decode_standard(digits: &str) -> Result<Mnemonic, Error> {
    let s = digits.trim();
    if !matches!(s.len(), 48 | 96) || !s.bytes().all(|b| b.is_ascii_digit()) {
        return Err(Error::SeedQr("standard form is 48 or 96 decimal digits"));
    }
...
pub fn decode_compact(bytes: &[u8]) -> Result<Mnemonic, Error> {
    if !matches!(bytes.len(), 16 | 32) {
        return Err(Error::SeedQr("compact form is 16 or 32 entropy bytes"));
    }
...
/// SeedQR, 16/32-byte payloads are compact.
pub fn decode(payload: &[u8]) -> Result<Mnemonic, Error> {
    if matches!(payload.len(), 48 | 96) && payload.iter().all(|b| b.is_ascii_digit()) {
