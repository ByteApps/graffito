...
<p align="center">
  ...
  &nbsp;
  <img src="screenshots/compose.png" alt="Compose — set your defaults once, then see exactly what a note will use at a glance" width="290">
  &nbsp;
...
- **Compose that gets out of your way** — set your defaults once in Settings (visibility, fee tier, gift amount, which wallet pays, coin selection, quantum encryption, passphrase cost) and Compose becomes just **To + your note**, with a row of small pills showing exactly what this note will use — privacy, quantum protection, passphrase, fee rate, gift, and paying source. Tap any pill to change just that one setting for this note only; it reverts to your default the moment you're done. Full **coin control**, a custom change address, and a **gift amount** to send chosen sats along with a directed note are still there, one tap behind the pills.
- **Quantum-resistant private notes (optional)** — layer a generated 132-bit passphrase and/or ML-KEM hybrid encryption (512/768/1024, RFC 9980-compatible key import) over a directed note's standard end-to-end encryption, to one recipient or several at once — each recipient gets their own ML-KEM layer at their own key's strength, and a shared passphrase can cover the whole note. A live security label on the compose screen tells you exactly what protection the note will carry, and Settings manages your quantum keys — derived from your seed, so they recover with your words.
...
- **Your infrastructure or theirs** — pick your Bitcoin node, per network, right in Settings: a public Esplora-compatible endpoint (mempool.space, Blockstream), your own Bitcoin Core node, or your own Electrum server (electrs — the same server Sparrow connects to) — plus your block explorer.
...
The app and the Passport Prime app share one `notes-core` crate (it lives in this repo; the Prime app pins it) — envelope, encryption, and transaction signing are the same code, so notes from either app are interchangeable on-chain. The [web viewer](https://byteapps.com/graffito/companion/) renders both.
